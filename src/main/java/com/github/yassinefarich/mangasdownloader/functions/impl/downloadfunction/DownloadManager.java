...
import static com.github.yassinefarich.mangasdownloader.functions.impl.downloadfunction.Constants.*;
...
public class DownloadManager {
    ...
    private Optional<File> makeDownloadDestination(List<Integer> chaptersPageSequnce) {
        ...
        StringBuilder downloadFolder = new StringBuilder(applicationProperties.getProperty(DOWNLOAD_PATH).get());
    ...
    private Optional<String> applyModificationOn(Optional<String> mangasImageURL) {
        String modificationTemplate = applicationProperties.getProperty(DOWNLOAD_URL_MODIF).get();
    ...
    private Optional<String> findXPATHExpressionInPage(String pageURL) {
        ...
        try {
            ...
            Optional<String> fullDownlodPath = Optional.fromJavaUtil(CollectionUtils.arrayToList(tagNode
                    .evaluateXPath(applicationProperties.getProperty(PARSER_XPATH).get()))
                    .stream().findFirst());
